...
- We improved Toga's [handling of the `asyncio` event loop on Windows](https://github.com/beeware/toga/pull/3270). This change ensures that WinForms apps always clean up the event loop on exit; previously, it was possible for warnings to be raised on application exit, caused by open sockets and <nospell>un-`await`ed</nospell> co-routines.
